...
public class CopyStatementPlanner {
    ...
    public Plan planCopyFrom(CopyFromAnalyzedStatement analysis, Planner.Context context) {
        ...
        SourceIndexWriterProjection sourceIndexWriterProjection = new SourceIndexWriterProjection(
            table.ident(),
            partitionIdent,
            table.getReferenceInfo(DocSysColumns.RAW),
            table.primaryKey(),
            table.partitionedBy(),
            partitionValues,
            table.clusteredBy(),
        ...
        for (ColumnIdent primaryKey : table.primaryKey()) {
            toCollect.add(table.getReferenceInfo(primaryKey));
        }

        // add partitioned columns (if not part of primaryKey)
        Set<Reference> referencedReferences = new HashSet<>();
        for (String partitionedColumn : partitionedByNames) {
            Reference reference = table.getReferenceInfo(ColumnIdent.fromPath(partitionedColumn));
            Symbol symbol;
            if (reference instanceof GeneratedReference) {
                symbol = ((GeneratedReference) reference).generatedExpression();
                referencedReferences.addAll(((GeneratedReference) reference).referencedReferenceInfos());
            } else {
                symbol = reference;
            }
        ...
        if (clusteredByPrimaryKeyIdx == -1 && table.clusteredBy() != null && !DocSysColumns.ID.equals(table.clusteredBy())) {
            toCollect.add(table.getReferenceInfo(table.clusteredBy()));
        }
        // add _raw or _doc
        if (table.isPartitioned() && analysis.partitionIdent() == null) {
            toCollect.add(table.getReferenceInfo(DocSysColumns.DOC));
        } else {
            toCollect.add(table.getReferenceInfo(DocSysColumns.RAW));
        }
